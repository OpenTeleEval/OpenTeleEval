...
**OpenTeleEval** bridges this gap. It is a comprehensive benchmark comprising **12 evaluation sets** with **22,678 curated samples**. It evaluates LLMs across a synergistic hierarchy:
...
OpenTeleEval consists of 12 tasks categorized into two main levels.
...
This repository is **dual-licensed by directory** — there is no single license covering
the whole repo. A file's license follows its top-level directory:

| Path | License | Full text |
|:--|:--|:--|
| `code/**` (and root docs) | Apache License 2.0 | [CODE_LICENSE.md](./CODE_LICENSE.md) |
| `datasets/**` | CDLA-Permissive-2.0 | [DATASET_LICENSE.md](./DATASET_LICENSE.md) |

- Where a source file carries its own SPDX/copyright header, that header takes precedence.
- Evaluation **results** derived from the datasets are unrestricted
  (CDLA-Permissive-2.0 §3.1).
- See [LICENSE](./LICENSE) for the authoritative statement and [NOTICE](./NOTICE) for
  third-party attribution (OpenCompass, HuggingFace).
