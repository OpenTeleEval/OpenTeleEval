...
Evaluations are launched **from the `code/` directory**. Dataset `path` values in the
configs are written as `../datasets/...`, which resolve to the **top-level
[`datasets/`](../datasets/) directory** (one level above `code/`). Keep this convention
when you add new configs — do **not** place raw data under `code/` (see
[Licensing & File Placement](#licensing--file-placement)).
...
│       ├── BasicKnowledge.py
│       └── CoreNetwork.py
├── datasets/                            # raw JSON evaluation data (subsets only) — CANONICAL data store
...
│       ├── Solution_Generation/
│       └── Tool_Invocation/
├── LICENSE                              # authoritative directory → license map
├── CODE_LICENSE.md                      # Apache-2.0 full text (code/)
├── DATASET_LICENSE.md                   # CDLA-Permissive-2.0 full text (datasets/)
├── NOTICE                               # third-party attribution (OpenCompass, HuggingFace)
...
| `datasets/` | Top-level raw JSON samples (subsets only — see "Obtaining the Complete Dataset" below). The single canonical copy; configs reference it via `../datasets/...`. |
| `code/run.py` | CLI entry. Runs the full pipeline: **infer → eval → summarize**. |

---

## Licensing & File Placement

This repository is **dual-licensed by directory** (see [`LICENSE`](../LICENSE) for the
authoritative statement):

| Path | License |
|------|---------|
| `code/**` (and root documentation) | [Apache License 2.0](../CODE_LICENSE.md) |
| `datasets/**` | [CDLA-Permissive-2.0](../DATASET_LICENSE.md) |

To keep this mapping unambiguous, follow these rules when contributing:

1. **Raw data (JSON / any benchmark samples) always goes under `datasets/`, never under
   `code/`.** Placing data inside `code/` would make it look Apache-licensed and create a
   licensing conflict.
2. **Code, configs, prompts, and scripts always go under `code/`.**
3. In a dataset config, reference data with a path relative to the run directory
   (`code/`), i.e. `path="../datasets/..."`.
4. New Python files you author should carry the header
   `# SPDX-License-Identifier: Apache-2.0` (see existing files). Do **not** overwrite or
   remove copyright/SPDX headers on files derived from upstream projects (OpenCompass,
   HuggingFace) — extend the attribution in [`NOTICE`](../NOTICE) instead.
5. Evaluation **Results** (models, metrics, insights derived from the data) are not
   restricted by the data license (CDLA-Permissive-2.0 §3.1).

---

## Dataset Format

All evaluation sets live under the top-level [`datasets/`](../datasets/) directory,
organized by Knowledge Comprehension vs Knowledge Application.

### Knowledge Comprehension

**Single-/multi-choice (Basic Knowledge / 5G / 3GPP / Fault Maintenance / Network Optimization / Wired Network)**

Each file is a JSON object with a `questions` list. A sample from `datasets/Knowledge_Comprehension/Basic Theory/Basic_Knowledge/basic_knowledge.json`:
...
Create directories under the **top-level** `datasets/` (never under `code/`):

```
datasets/Knowledge_Application/Parameter_Check/parameter_check.json
```
...
Create `code/configs/datasets/param_check/param_check_gen.py`. Note the data `path` is
written relative to the run directory (`code/`), so it points one level up to the
top-level `datasets/`:
...
1. Open the JSON file under `datasets/Knowledge_Comprehension/...` or `datasets/Knowledge_Application/...`.
...
1. Add the raw data under the top-level `datasets/` (mirror the existing `Knowledge_Comprehension` / `Knowledge_Application` split).
2. Add a loader + evaluator in `code/opencompass/datasets/<task>.py` and re-export it in `__init__.py`.
3. Add a reusable config in `code/configs/datasets/<task>/` (with `path="../datasets/..."`).
